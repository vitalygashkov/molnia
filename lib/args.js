...
const fromCamelToKebab = () => {
  const keys = Object.keys(options);
  const result = {};
  for (const key of keys) result[key.replace(/[A-Z]/g, '-$&').toLowerCase()] = options[key];
  return result;
};
...
const getArgs = () => {
  ...
  if (isCli) {
    const { values, positionals } = parseArgs({ options: fromCamelToKebab(options), allowPositionals: true });
