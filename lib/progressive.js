...
const downloadProgressive = async (url, options, contentLength) => {
  ...
  const filename = basename(options.output);
  ...
  console.time(`File ${filename} downloaded`);
  for (let i = 0; i < ranges.length; i += 1) {
    const [start, end] = ranges[i];
    const size = end - start + 1;
    const headers = { ...(options.headers || {}), 'User-Agent': options.userAgent };
    const downloadChunkParams = { id: i, range: [start, end], url, headers, output: options.output };
    queue.push(downloadChunkParams);
  }
  await new Promise((resolve) => (queue.drain = () => resolve()));
  console.timeEnd(`File ${filename} downloaded`);
};
