...
const fetchRange = async ({ id, range, url, output }) => {
  ...
  console.time('Download and save chunk ' + id);
  await fetchWithStream(url, options, createStream);
  console.timeEnd('Download and save chunk ' + id);
  console.time('Rename chunk ' + id);
  ...
  console.timeEnd('Rename chunk ' + id);
  return stats;
};
  ...
  for (let i = 0; i < ranges.length; i += 1) {
    ...
    if (!chunkStats) {
      ...
      if (chunkStats.size > size)
        throw new Error(`Expecting maximum chunk size of ${size} but got: ${chunkStats.size}`);
      if (chunkStats.size === size) {
        // Chunk already downloaded, resume downloading
        console.log('Resume is not implemented yet');
      } else {
        queue.push(fetchRangeOptions);
      }
    }
