...
const createClient = (options = {}) => {
  ...
  const connectTimeout = 300e3; // 5 minutes instead of default 10 seconds
  const agent = options.proxy
    ? new ProxyAgent({ uri: options.proxy, maxRedirections, connectTimeout })
    : new Agent({ maxRedirections, connectTimeout });
