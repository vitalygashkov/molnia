...
const downloadSegments = async (data, options = {}) => {
  ...
  const isMetaSegmentsCountMismatch =
  ...
  const queue = createQueue(async (params) => {
    if (signal?.aborted) return;
    const error = await save(params.saveOptions);
  ...
  progress.setCurrent(meta.bytesDownloaded);
