...
const createStream = ({ headers, opaque, statusCode }) => {
  ...
  onSegmentHeaders?.(headers, url, statusCode);
...
const downloadSegments = async (urls, options = {}) => {
  ...
  const TEMP_FOLDER = '.esor';
  const tempPath = path.join(tempDir, TEMP_FOLDER);
  ...
  const onSegmentHeaders = (headers, url, statusCode) => {
