...
const downloadSegments = async (data, options = {}) => {
  ...
  const queue = createQueue(async ({ index, saveOptions, segmentBytes }) => {
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        if (!signal?.aborted) queue.push({ index, saveOptions, segmentBytes });
      } else {
        onError?.(
          error,
    ...
    if (segmentBytes) {
      meta.bytesDownloaded = (meta.bytesDownloaded || 0) + segmentBytes;
    }
    await writeMeta(metaPath, meta);
  }, connections);
  ...
  const createOnHeaders = (index) => (headers) => {
