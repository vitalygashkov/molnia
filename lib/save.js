...
const save = async ({
  ...
  const options = { method, headers, dispatcher, maxRedirections: 3 };
  const { start } = withRange(options, range);
  const response = await request(url, options).catch((error) => error);
  if (response instanceof Error) return onError?.(response, 'Request error');
